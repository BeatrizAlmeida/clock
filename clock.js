...
function drawClock(place) {
  ...
  img.decode().then(() => {
    ...
    logo.decode().then(() => {
      // Translate the center of the logo
      // to the center of the canvas.
      let size = imgSize(logo.width -280, logo.height, 0.9 * clockRadius);
      var coord = translate(scale(size, [-1 / 2, -1 / 2]), center);
 ...
 */
drawClock.decimals = Array.from(Array(25), (_, i) => {
  return { txt: String(i), c: white2 };
}).slice().reverse();
drawClock.decimals.pop();
...
var runAnimation = (() => {
  ...
  function drawHandles() {
    ...
    clock_handles[0].time2Angle = fiveMin * -1 * (+hours12 + minutes / 60);
    clock_handles[1].time2Angle = oneMin *  -1 *(+minutes + seconds / 60);
    clock_handles[2].time2Angle = oneMin *  -1 *seconds;
    clock_handles[3].time2Angle = fiveMin *  -1 *(+hours + minutes / 60) * 0.5;
